<?php

require_once 'class/Constant.php';
require_once 'class/General.php';
require_once 'class/DbMysql.php';
require_once 'class/License.php';

$apiName = 'license';
$isTransaction = false;
$formData = array('success'=>false, 'result'=>'', 'error'=>'', 'errmsg'=>'');
$result = '';
date_default_timezone_set("Asia/Kuala_Lumpur");

$fnMain = new License();

try {
    $fnMain->isLogged = Constant::$isLogged;
    DbMysql::$isLogged = Constant::$isLogged;

    $requestMethod = $_SERVER['REQUEST_METHOD'];
    $fnMain->logDebug('API', $apiName, __LINE__, 'Request method = '.$requestMethod.', URL = '.$_SERVER['REQUEST_URI']);
    $urlArr = $fnMain->getUrlArr($_SERVER['REQUEST_URI'], $apiName);

    DbMysql::connect();
    if (isset($urlArr[1]) && $urlArr[1] === 'ext') {
        array_shift($urlArr);
    } else {
        $fnMain->checkJwt(apache_request_headers());
    }

    if ('GET' === $requestMethod) {
        if (!isset($urlArr[1])) {
            $result = $fnMain->getList();
        } else if (is_numeric($urlArr[1])) {
            $result = $fnMain->get(intval($urlArr[1]));
        } else {
            throw new Exception('[line: ' . __LINE__ . '] - Wrong GET Request');
        }
        $formData['result'] = $result;
        $formData['success'] = true;
    }
    else if ('POST' === $requestMethod) {
        // Accept JSON body (preferred) or form-encoded fallback
        $raw = file_get_contents("php://input");
        $params = json_decode($raw, true);
        if (!is_array($params) || empty($params)) { $params = $_POST; }
        // Normalize fileUpload structure if present as flat fields
        $file = array();
        if (isset($params['fileUpload']) && is_string($params['fileUpload'])) {
            $decoded = json_decode($params['fileUpload'], true);
            if (is_array($decoded)) { $file = $decoded; }
        }
        // Accept serialized keys fileUpload[name] or flat name/filename
        $keys = array('name','filename','size','type','data','width','height');
        $hasFlat = true; $hasNested = true;
        foreach ($keys as $k) {
            if (!isset($params[$k])) { $hasFlat = false; }
            if (!isset($params['fileUpload['.$k.']'])) { $hasNested = false; }
        }
        if (empty($file)) {
            if ($hasNested) {
                foreach ($keys as $k) { $file[$k] = $params['fileUpload['.$k.']']; }
            } else if ($hasFlat) {
                foreach ($keys as $k) { $file[$k] = $params[$k]; }
            }
        }
        if (!empty($file)) {
            $params['fileUpload'] = $file;
        }

    DbMysql::beginTransaction();
        $isTransaction = true;

        $fnMain->insert($params);
        // Optionally: $fnMain->updateVersion(<id>); $fnMain->saveAudit(<id>, $fnMain->licenseTitle);

        DbMysql::commit();
        $formData['result'] = $result;
        $formData['success'] = true;
    }
    else if ('PUT' === $requestMethod) {
        $putData = file_get_contents("php://input");
        $params = json_decode($putData, true);
        if (!is_array($params) || empty($params)) {
            // Fallback for x-www-form-urlencoded
            $params = [];
            parse_str($putData, $params);
        }
        if (!isset($urlArr[1])) {
            throw new Exception('[line: ' . __LINE__ . '] - Empty url parameter 1');
        }

        // Normalize fileUpload same as in POST
    if (isset($params['fileUpload']) && is_string($params['fileUpload'])) {
            $decoded = json_decode($params['fileUpload'], true);
            if (is_array($decoded)) { $params['fileUpload'] = $decoded; }
        } else {
            $keys = array('name','filename','size','type','data','width','height');
            $hasFlat = true; $hasNested = true; $file = array();
            foreach ($keys as $k) { if (!isset($params[$k])) { $hasFlat = false; } }
            foreach ($keys as $k) { if (!isset($params['fileUpload['.$k.']'])) { $hasNested = false; } }
            if ($hasNested) { foreach ($keys as $k) { $file[$k] = $params['fileUpload['.$k.']']; } }
            else if ($hasFlat) { foreach ($keys as $k) { $file[$k] = $params[$k]; } }
            if (!empty($file)) { $params['fileUpload'] = $file; }
        }

        DbMysql::beginTransaction();
        $isTransaction = true;
        $fnMain->update(intval($urlArr[1]), $params);
        // Optionally: $fnMain->updateVersion(<id>); $fnMain->saveAudit(<id>, $fnMain->licenseTitle);
        DbMysql::commit();

        $formData['result'] = $result;
        $formData['success'] = true;
    }
    else if ('DELETE' === $requestMethod) {
        if (!isset($urlArr[1])) {
            throw new Exception('[line: ' . __LINE__ . '] - Empty url parameter 1');
        }

        DbMysql::beginTransaction();
        $isTransaction = true;
        $fnMain->delete(intval($urlArr[1]));
        // Optionally: $fnMain->updateVersion(<id>); $fnMain->saveAudit(<id>, $fnMain->licenseTitle);
        DbMysql::commit();

        $formData['result'] = $result;
        $formData['success'] = true;
    }
    else {
        throw new Exception('[line: ' . __LINE__ . '] - Wrong Request Method');
    }
    DbMysql::close();
} catch (Exception $e) {
    try {
        if ($isTransaction) {
            DbMysql::rollback();
        }
        DbMysql::close();
    } catch (Exception $ex) {
        $fnMain->logError('API', $apiName, __LINE__, $e->getMessage());
    }
    $formData['error'] = strpos($e->getMessage(), '] -') ? substr($e->getMessage(), strpos($e->getMessage(), '] -') + 4) : substr($e->getMessage(), strripos($e->getMessage(), '] ') + 2);
    $formData['errmsg'] = $e->getCode() === 31 ? $formData['error'] : Constant::$err['default'];
    $fnMain->logError('API', $apiName, __LINE__, $e->getMessage());
}

echo json_encode($formData);
