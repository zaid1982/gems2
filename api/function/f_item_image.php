...
class Class_item_image {
     ...
     * @throws Exception
     */
    public function getItemImageList ($itemId) {
        try {
     ...
     * @throws Exception
     */
    public function getItemImage ($itemImageId) {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);
     ...
     * @throws Exception
     */
    public function addItemImage ($itemId, $uploadId) {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);
            $this->fn_general->checkEmptyParams(array($itemId, $uploadId));
            return Class_db::getInstance()->db_insert('ref_item_image', array('item_id'=>$itemId, 'upload_id'=>$uploadId));
        }
        catch(Exception $ex) {
            $this->fn_general->log_error(__CLASS__, __FUNCTION__, __LINE__, $ex->getMessage());
            throw new Exception($this->get_exception('0005', __FUNCTION__, __LINE__, $ex->getMessage()), $ex->getCode());
