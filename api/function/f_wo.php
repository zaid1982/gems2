...
class Class_wo {
    ...
    public function get_pending_task_m ($searchText='', $woType='') {
        try {
            ...
            $result = array();
            $arr_dataLocal = Class_db::getInstance()->db_select('mw_wo_pending_m', array(), 'wfl_task.task_id DESC', '100', null, array('user_id'=>$this->userId, 'search_text'=>$searchText, 'wo_type'=>$woType));
            foreach ($arr_dataLocal as $dataLocal) {
                $row_result['woTaskId'] = $dataLocal['wo_task_id'];
    ...
    public function get_complaint_details_m () {
        try {
            ...
            $result['woTaskId'] = $dataLocal['wo_task_id'];
            $result['woTaskNo'] = $dataLocal['wo_task_wr_confirm'] === '1' ? $dataLocal['wo_task_no'] : '-';
    ...
    public function get_wo_task_dashboard_list ($clientId='', $siteId='', $year='', $month='', $isPending=false) {
        try {
            ...
            foreach ($arr_dataLocal as $dataLocal) {
                $row_result['woTaskId'] = $dataLocal['wo_task_id'];
                $row_result['woTaskNo'] = $dataLocal['wo_task_wr_confirm'] === '1' ? $dataLocal['wo_task_no'] : '-';
