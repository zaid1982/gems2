...
class Class_wo {
    ...
    public function get_pending_task_m ($searchText='') {
        try {
            ...
            $result = array();
            $arr_dataLocal = Class_db::getInstance()->db_select('mw_wo_pending_m', array(), 'wfl_task.task_id DESC', '100', null, array('user_id'=>$this->userId, 'search_text'=>$searchText));
            foreach ($arr_dataLocal as $dataLocal) {
                $row_result['woTaskId'] = $dataLocal['wo_task_id'];
                $row_result['woTaskNo'] = $dataLocal['wo_task_no'];
                $row_result['woTaskType'] = $dataLocal['wo_task_type_desc'];
     ...
     * @throws Exception
     */
    public function submit_wr_check ($transactionId='', $signatureId='') {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering ' . __CLASS__);
            ...
            Class_db::getInstance()->db_update('wo_task', array('wo_task_wr_checked_by'=>$this->userId, 'wo_task_time_wr_checked'=>'Now()', 'wo_task_status'=>'28'), array('wo_task_id'=>$this->woTaskId));
