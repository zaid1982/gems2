...
class Class_store {
    ...
    public function __set($property, $value ) {
        if (property_exists($this, $property)) {
            ...
            throw new Exception($this->get_exception('0004', __FUNCTION__, __LINE__, 'Set Property not exist ['.$property.']'));
        }
    }
    ...
    public function __isset($property ) {
        if (property_exists($this, $property)) {
            ...
            throw new Exception($this->get_exception('0004', __FUNCTION__, __LINE__, 'IsSet Property not exist ['.$property.']'));
        }
    }
     ...
     * @throws Exception
     */
    public function getStoreListBySite ($siteId) {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);
            $this->fn_general->checkEmptyParams(array($siteId));
            return Class_db::getInstance()->db_select2('vw_store', array('site_id'=>$siteId));
        }
        catch(Exception $ex) {
            $this->fn_general->log_error(__CLASS__, __FUNCTION__, __LINE__, $ex->getMessage());
            throw new Exception($this->get_exception('0005', __FUNCTION__, __LINE__, $ex->getMessage()), $ex->getCode());
