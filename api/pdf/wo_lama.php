...
require_once __DIR__.'/pdf_error_page.php';

if (!class_exists('MYPDF_wo')) {
...
class Class_pdf_wo {
    ...
    /**
     * Persist exception details as a downloadable PDF (same folder / sys_pdf flow).
     */
    private function create_error_pdf(Throwable $ex) {
        $folder_code = floor(intval($this->woTaskId) / 1000);
        $folder = 'pdf/wo/'.$folder_code;
        $folderPath = __DIR__.'/wo/'.$folder_code;
        $filename = 'wo_error_'.substr((10000000 + intval($this->woTaskId)), 1).'.pdf';
        $absolutePath = $folderPath.'/'.$filename;

        $woTaskNo = '';
        try {
            $woTaskNo = Class_db::getInstance()->db_select_col('wo_task', array('wo_task_id' => $this->woTaskId), 'wo_task_no');
        } catch (Throwable $ignored) {
        }

        gems_pdf_write_error_page(
            $absolutePath,
            'GEMS WO PDF Error',
            gems_pdf_error_lines($ex, array(
                'WO Task ID' => $this->woTaskId,
                'WO Task No' => $woTaskNo,
                'Source' => 'wo_lama.php',
            ))
        );
        ...
        if (empty($pdfId)) {
            ...
            Class_db::getInstance()->db_update(
                'sys_pdf',
                array('pdf_filename' => $filename, 'pdf_type' => 'wo', 'pdf_folder' => $folder, 'pdf_timeCreated' => 'Now()'),
                array('pdf_id' => $pdfId)
            );
        }
        Class_db::getInstance()->db_update(
        ...
        return array(
            'pdfId' => $pdfId,
            'woTaskNo' => $woTaskNo !== '' ? $woTaskNo : null,
            'errorPdf' => true,
        );
    }
    ...
    public function create_pdf () {
        try {
            ...
            try {
                // Surface the exception inside a PDF the UI can open.
                return $this->create_error_pdf($ex);
            } catch (Throwable $inner) {
                $msg = $ex->getMessage().' | error-pdf failed: '.$inner->getMessage();
                if ($this->fn_general) {
                    $this->fn_general->log_error(__CLASS__, __FUNCTION__, __LINE__, $msg);
                }
                throw new Exception($this->get_exception('0051', __FUNCTION__, __LINE__, $msg), 31);
            }
        }
    }
}
