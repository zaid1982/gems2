 ...
 * sessionStorage and sent as X-Api-Key on fetch() calls. Direct links to
 * .php endpoints get ?api_key= appended automatically.
 */
(function () {
    ...
    function decoratePhpLinks() {
        const key = getKey();
        if (!key) {
            return;
        }
        document.querySelectorAll('a[href*=".php"]').forEach(function (anchor) {
            try {
    ...
    function showUnlockOverlay() {
        ...
        overlay.innerHTML = [
            ...
            'Enter the maintenance API key configured in <code>api/library/config.ini</code>',
            ' (<code>[maintenance] api_key</code>) or <code>MAINTENANCE_API_KEY</code> in <code>.env</code>.',
            '</p>',
