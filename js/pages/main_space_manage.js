function MainSpaceManage(){
  ...
  this.init=function(){
    ...
    oResv=$('#dtSpmResv').DataTable({ bLengthChange:false,bFilter:false,aaSorting:[[1,'asc']], language:_DATATABLE_LANGUAGE, fnRowCallback:function(nRow,aData,iDisplayIndex){ const info=oResv.page.info(); $('td',nRow).eq(0).html(info.page*info.length + (iDisplayIndex+1)).attr('data-label','#'); $('td',nRow).eq(1).attr('data-label','Start'); $('td',nRow).eq(2).attr('data-label','End'); $('td',nRow).eq(3).attr('data-label','Requester'); $('td',nRow).eq(4).attr('data-label','Status'); $('td',nRow).eq(5).attr('data-label','Actions'); }, aoColumns:[ 
      ...
      {mData:'reservationStatus', mRender:function(d){ const status=(d||'').toLowerCase(); let badge='spm-status-badge '; if(status==='approved'){ badge+='approved'; }else if(status==='pending'){ badge+='pending'; }else if(status==='cancelled'||status==='canceled'){ badge+='cancelled'; }else if(status==='rejected'){ badge+='rejected'; }else{ badge+='pending'; } return '<span class="'+badge+'">'+d+'</span>'; }}, 
      {mData:null,bSortable:false,sClass:'text-center action-cell', mRender:function(d,t,row){ const disabled = (row.reservationStatus||'').toUpperCase()==='CANCELED'; return '<div class="spm-action-btn-group"><button class="btn-spm-action btn-view btnSpmViewResv" data-id="'+row.reservationId+'" title="View details"><i class="fas fa-eye"></i></button><button class="btn-spm-action btn-cancel btnSpmCancelResv" '+(disabled?'disabled':'')+' data-id="'+row.reservationId+'" title="Cancel reservation"><i class="fas fa-ban"></i></button></div>'; }} 
    ] });
    // Events
